...
namespace lab{
	...
	public:
		...
		void rotateFPSoX(float angle){
			//rotatie
			//TODO
			up = up * glm::cos(angle) + forward * glm::sin(angle);
		...
		void rotateFPSoY(float angle){
			//rotatie
			//TODO
			forward = forward * glm::cos(angle) + right * glm::sin(angle);
			right = glm::cross(forward, up);
		...
		void rotateFPSoZ(float angle){
			//rotatie
			//TODO
			up = up * glm::cos(angle) + right * glm::sin(angle);
			right = glm::cross(forward, up);
		...
		void rotateTPSoX(float angle, float distance){
			//rotatie
			//TODO
			glm::vec3 old_for = forward;
			rotateFPSoX(angle);
			position = position + distance * old_for - distance * forward;
